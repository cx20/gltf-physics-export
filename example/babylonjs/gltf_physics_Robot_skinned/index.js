...
const MODEL_ROOT = 'https://raw.githubusercontent.com/eoineoineoin/glTF_Physics/master/samples/Robot_skinned/';
const MODEL_FILE = 'Robot_skinned.glb';
...
async function createScene() {
    ...
    if (allMeshes.length > 0) {
        ...
        const radius = BABYLON.Vector3.Distance(bounds.min, bounds.max) * 0.45;
        camera.setTarget(center);
        camera.radius = Math.max(radius, 8);
    }

    scene.registerBeforeRender(function () {
        camera.alpha += 0.0012 * scene.getAnimationRatio();
    });
